...
let user_name: string = "Beau Iyere";
...
let guests: string[] = ["Beau Iyere", "Andre", "Ella"];

// Types for function
// let KillBill : (name: string) => string; <====== first way

const KillBill = function (name: string): string {
    return name.toUpperCase()
};

const passBill = function (name: string): void {
...
let myGuest : (string | number | boolean)[] = ["test", 22, false];
...
const f1: unique symbol = Symbol("f"); // a variable whose type is a unique symbol must be prefixed with the const declaration
...
let e: { b: number; c?: string; [key: number]: boolean };

//using the readonly modifier

let user: { readonly firstName: string } = { firstName: "Beau" };
